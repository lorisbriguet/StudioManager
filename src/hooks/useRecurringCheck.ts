...
// Module-level guard: the startup effect, the window-focus re-check, and the
// manual "Generate now" button all funnel here — overlapping runs would
// double-generate drafts.
let inFlight = false;

// Local day of the last completed check. The app is often left running for
// weeks, so the focus listener re-checks when this no longer matches today.
let lastRunDay: string | null = null;

/**
 * Generate draft invoices for every due recurring template.
 * Returns the number of drafts generated (0 if nothing due or already running).
 */
export async function runRecurringCheck(qc: QueryClient): Promise<number> {
    ...
    for (const tmpl of templates) {
      ...
      try {
        ...
        while (nextDue <= today && steps < MAX_CATCHUP_PER_TEMPLATE) {
          ...
          // persist regenerates exactly one period on the next run (a
...
export function useRecurringCheck() {
  ...
  useEffect(() => {
    if (!ran.current) {
      ran.current = true;
      void runRecurringCheck(qc);
    }

    const onFocus = () => {
      if (lastRunDay !== todayLocalISO()) void runRecurringCheck(qc);
    };
